...
def SetXmlx(fichero, hoja ):
    wb = load_workbook(filename= fichero )    #'/home/peli/Descargas/MasterArticulosHupa.xlsx'
    print(wb.get_sheet_names())  # muestra todas las hojas del fichero
    ws = wb.active
    ws = wb.get_sheet_by_name( hoja )  # abre la hoja apuntada  'Articulos'
    fila = []
    Filas_ = []
    # all_rows = ws.rows.count
    nfilas = ws.max_row
    ncol = ws.max_column + 1
    print(" Numero de lineas en el excell " + str(nfilas))
    print(" Numero de columnas en el excell " + str(ncol))
    ind = 0
    for i in range(1, nfilas):
        for j in range(ncol):

            if i > 1 and j > 0:
                tmp = ws.cell(row=i, column=j).value
                fila.insert(ind, tmp)
        ...
        if i > 1 and j > 0:
            Filas_.append(fila)
            fila = []
    #obunicode = unicode(Filas_).encode('utf-8') # Se queda en texto plano
    sql = "INSERT INTO Articulos(  nombre , codigo , cod_barras , fk_familia , imputacion ) VALUES "
    for i in Filas_:
        sql += "('" + str(i[2]).encode('ascii', 'ignore').decode('ascii') +"','"
        sql += str(i[1]) + "','"
        sql += str(i[1]) + "','"
        sql += "4" + "','"
        sql += "0" + "'),"
    ...
    print( sql )
    print(len(Filas_))
    ret = client.service.ExecCommandHex( Getordinal( sql ) )
    print( "Resultado Insert: " + str(ret) )


def MostrarSelect( datos ):
    if( datos != None):
        for row in datos:
            for itm in row:
                itm = itm.encode("ascii",errors='replace')
                print( itm )
            print( '____________')
    return len( v )
...
res = client.service.RetornarConsultaHex( Getordinal("SELECT * FROM articulos WHERE nombre like '%esparadrapo%'") )
v = ConvLista( res )
ret = MostrarSelect( v )
print("Filas devueltas " + str( ret ) + " filas.")


#res = client.service.ExecCommandHex( Getordinal("DELETE FROM  articulos"))
#print("Ficheros borrados: " + str(res))
...
#res = client.service.ExecCommandHex( Getordinal("delete from sqlite_sequence where name= 'Articulos'"))
#print(res)


#res =SetXmlx('/home/peli/Descargas/MasterArticulosHupa.xlsx' , 'Articulos' )
#print( res)
